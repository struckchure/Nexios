...
from nexio.orm import Model, nexio.orm, connections
from nexio.orm.backends.base.config_generator import generate_config as _generate_config
from nexio.orm.exceptions import DBConnectionError, OperationalError
...
_nexio.orm_TEST_DB = "sqlite://:memory:"
...
async def _init_db(config: dict) -> None:
    ...
    await nexio.orm.init(config)
    try:
        await nexio.orm._drop_databases()
    except (DBConnectionError, OperationalError):  # pragma: nocoverage
        pass

    await nexio.orm.init(config, _create_db=True)
    await nexio.orm.generate_schemas(safe=False)
...
def _restore_default() -> None:
    ...
    nexio.orm._init_apps(_CONFIG["apps"])
    nexio.orm._inited = True
...
def initializer(
    ...
    global _CONNECTIONS
    global _LOOP
    global _nexio.orm_TEST_DB
    global _MODULES
    global _CONN_CONFIG
    _MODULES = modules
    if db_url is not None:  # pragma: nobranch
        _nexio.orm_TEST_DB = db_url
...
def finalizer() -> None:
    ...
    _restore_default()
    loop = _LOOP
    loop.run_until_complete(nexio.orm._drop_databases())
...
def env_initializer() -> None:  # pragma: nocoverage
    ...
    """
    modules = str(_os.environ.get("nexio.orm_TEST_MODULES", "tests.testmodels")).split(",")
    db_url = _os.environ.get("nexio.orm_TEST_DB", "sqlite://:memory:")
    app_label = _os.environ.get("nexio.orm_TEST_APP", "models")
    if not modules:  # pragma: nocoverage
        raise Exception("nexio.orm_TEST_MODULES envvar not defined")
    initializer(modules, db_url=db_url, app_label=app_label)


class SimpleTestCase(unittest.IsolatedAsyncioTestCase):
    """
    The nexio.orm base test class.
...
class IsolatedTestCase(SimpleTestCase):
    ...
    If you define a ``nexio.orm_test_modules`` list, it overrides the DB setup module for the tests.
    """

    nexio.orm_test_modules: Iterable[Union[str, ModuleType]] = []

    async def _setUpDB(self) -> None:
        await super()._setUpDB()
        config = getDBConfig(app_label="models", modules=self.nexio.orm_test_modules or _MODULES)
        await nexio.orm.init(config, _create_db=True)
        await nexio.orm.generate_schemas(safe=False)
...
class TruncationTestCase(SimpleTestCase):
    ...
    async def _tearDownDB(self) -> None:
        ...
        for app in nexio.orm.apps.values():
...
def init_memory_sqlite(
    ...
    .. code-block:: python3
        ...
        from nexio.orm import fields, models, run_async
        from nexio.orm.contrib.test import init_memory_sqlite
    ...
    def wrapper(func: AsyncFunc, ms: List[str]):
        ...
        async def runner(*args, **kwargs) -> T:
            await nexio.orm.init(db_url=MEMORY_SQLITE, modules={"models": ms})
            await nexio.orm.generate_schemas()
