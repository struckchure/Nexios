...
def before_request(func :Optional[Callable[[Request,Response], Awaitable[None]]],
                   *, 
                   log_level:Optional[str]=None, 
                   only_methods:Optional[List[str]] = None, 
                   for_routes:Optional[str]=None) -> None:
    """
...
def after_request(func:Optional[Callable[[Request,Response], Awaitable[None]]],
                  *, log_level :Optional[str]=None, 
                  only_methods:Optional[List[str]]=None, for_routes:Optional[List[str]]=None) -> None:
    """
    ...
    def decorator(handler :HandlerType) -> HandlerType:
        ...
        async def wrapper(*args :List[Any], **kwargs:Dict[str,Any]):
            ...
            if for_routes and req.url.path not in for_routes:
                return response
            if log_level:
                print(f"[{log_level}] After Request: {req.method} {req.url} - Status: {response._status_code}") #type: ignore
            if func:
...
def analytics(func :HandlerType) -> HandlerType:
    ...
    async def wrapper(*args :List[Any], **kwargs:Dict[str,Any]):
        ...
        print(f"Analytics: {req.method} {req.url} - {response._status_code} in {elapsed_time:.2f}s") #type:ignore
        return response
    return wrapper
...
def maintenance_mode(is_maintenance :bool = True):
    ...
    def decorator(handler :HandlerType) -> HandlerType:
        ...
        async def wrapper(*args :List[Any], **kwargs:Dict[str,Any]):
            res :Response =  args[-1] #type: ignore 
            if is_maintenance:
                return res.json({"error": "Service unavailable due to maintenance"}, status_code=503) #type: ignore #TODO:ad custom handler
            return await handler(*args, **kwargs)
        return wrapper
    return decorator
...
def use_for_route(route: str) -> None:
    ...
    def decorator(func: HandlerType) -> HandlerType:
        @wraps(func)
        async def wrapper_func(request :Request, response :Response, call_next :Callable[...,Awaitable[Response]]):
        ...
        @wraps(func)
        async def wrapper_klass(self :Any, request :Request, response :Response, call_next :Callable[...,Awaitable[Response]]):
            if re.match(route, request.url.path):
